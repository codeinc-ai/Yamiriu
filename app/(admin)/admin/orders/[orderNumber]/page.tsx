...
import { getAdminOrderDetail, type AdminOrderDetailItem } from "@/lib/queries/admin-orders";
...
function groupItems(items: AdminOrderDetailItem[]) {
  const standalone: AdminOrderDetailItem[] = [];
  const groups = new Map<string, AdminOrderDetailItem[]>();
...
export default async function AdminOrderDetailPage({
  ...
  const canRefund = can(user, "orders:refund");

  return (
    <div className="flex flex-col gap-6">
      <div>
        <Link href="/admin/orders" className="text-sm text-ink/60 hover:text-ink hover:underline">
          ← Back to orders
        </Link>
      </div>
      ...
      {order.trackingNumber ? (
        <div className="rounded-xl border border-ink/10 bg-white/60 p-6 text-sm">
          ...
          <p className="mt-2 text-ink/80">
            ...
            {trackingUrl ? (
              <a href={trackingUrl} target="_blank" rel="noopener noreferrer" className="font-medium text-terracotta hover:underline">
                {order.trackingNumber}
              </a>
            ) : (
      ...
      <div className="rounded-xl border border-ink/10 bg-white/60 p-6">
        ...
        <dl className="mt-5 flex flex-col gap-2 border-t border-ink/10 pt-4 text-sm">
          ...
          <div className="flex justify-between">
            ...
            <dd className="text-ink">{Number(order.shippingCost) === 0 ? "Free" : formatPkr(order.shippingCost)}</dd>
          </div>
        </dl>
      ...
      <div className="grid gap-6 sm:grid-cols-2">
        <div className="rounded-xl border border-ink/10 bg-white/60 p-6 text-sm">
          ...
          <p className="mt-2 text-ink/80">{PAYMENT_METHOD_LABELS[order.paymentMethod] ?? order.paymentMethod}</p>
          {order.providerTransactionId ? (
            <p className="mt-1 text-xs text-ink/60">Ref: {order.providerTransactionId}</p>
          ) : null}
          {order.codRefused ? (
            <p className="mt-2">
              <Badge variant="terracotta">Prior COD refusal on record</Badge>
            </p>
          ) : null}
        </div>
        <div className="rounded-xl border border-ink/10 bg-white/60 p-6 text-sm">
          <h2 className="font-display text-lg text-ink">Customer</h2>
          <p className="mt-2 text-ink/80">{order.customerEmail ?? "Guest"}</p>
          <p className="text-ink/80">{order.customerPhone}</p>
        </div>
        <div className="rounded-xl border border-ink/10 bg-white/60 p-6 text-sm sm:col-span-2">
